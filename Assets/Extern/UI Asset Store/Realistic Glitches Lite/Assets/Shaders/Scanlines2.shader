// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'
	...
	Shader "Hidden/Scanlines2" { 
			Properties { 
		_MainTex ("Base (RGB)", 2D) = "white" {} 
		_Intensity ("Black & White blend", 
		Range (0, 1)) = 0 
		_Color("Color - Full version only", Color) = (0,0,0,1) 
		_ValueX("LinesSize", 
		Range(1,10)) = 1 _ScrollSpeed("Scroll Speed", Float) = 1
}	SubShader { 
		Pass { 
			CGPROGRAM 
		...
		half _ValueX; half _Intensity; half _Value; half _ScrollSpeed;
		...
		fixed4 frag(v2f i) : COLOR {
	        ...
	        half offset = _Time.y * _ScrollSpeed;
	        i.uv.y += offset;
